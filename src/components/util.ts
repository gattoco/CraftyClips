...
const fetchClips = async (pBroadcasterId: number = 985378831, pNumberOfClips:number = FETCH_NUMBER_OF_CLIPS) => {
    try {
      await getTwitchAuthToken(); 
  
      const allClips = await getDataFromApi(TwitchApiEndpoints.CLIPS, {broadcaster_id: pBroadcasterId, first: pNumberOfClips});
  ...
  const fetchUsers = async (pBroadcasterName: string) => {
    try {
      ...
      const broadcaster = await getDataFromApi(TwitchApiEndpoints.USERS, { login: pBroadcasterName });
